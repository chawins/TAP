...
from tap.language_models import (
    GPT,
    APIModelLlama7B,
    APIModelVicuna13B,
    GeminiPro,
...
class AttackLLM:
    ...
    def get_attack(self, convs_list, prompts_list):
        ...
        for _ in range(self.max_n_attack_attempts):
            ...
            outputs_list = []
            for left in range(
                0, len(full_prompts_subset), MAX_PARALLEL_STREAMS
            ):
                right = min(
                    left + MAX_PARALLEL_STREAMS, len(full_prompts_subset)
                )
...
class TargetLLM:
    ...
    def get_response(self, prompts_list):
        ...
        convs_list = [
            common.conv_template(self.template) for _ in range(batchsize)
        ]
        full_prompts = []
        for conv, prompt in zip(convs_list, prompts_list):
            conv.append_message(conv.roles[0], prompt)
